...
if 'df_void' not in st.session_state:
    st.session_state.df_void = None
    ...
    if uploaded is not None:
        try:
            ...
            with st.spinner("Inférence YOLO..."):
                yolo_res = yolo_infer(st.session_state.img_rgb, conf=conf, iou=iou)
                st.session_state.instances = extract_instances(yolo_res)

            if st.session_state.instances:
                st.session_state.vis = draw_instances_overlay(st.session_state.img_rgb, st.session_state.instances)
                st.session_state.df_void = compute_void_rates(st.session_state.img_rgb, st.session_state.instances)
            else:
...
if page == "Analyse & CSV":
    ...
    if st.session_state.img_rgb is None:
        ...
        with col1:
            ...
            st.image(st.session_state.img_rgb, width="stretch")
        with col2:
            st.subheader("Prédictions YOLO (chips / voids)")
            st.image(st.session_state.vis, caption="Bleu = chip, Vert = void", width="stretch")

        st.subheader("Taux de void par chip")
        st.dataframe(st.session_state.df_void, use_container_width=True)

        # Résumé + export CSV
        mean_void = st.session_state.df_void["void_rate_percent"].mean()
        max_void = st.session_state.df_void["void_rate_percent"].max()

        st.markdown("**Résumé :**")
        st.write(f"- Taux de void moyen : **{mean_void:.2f}%**")
        st.write(f"- Taux de void max : **{max_void:.2f}%**")

        csv_bytes = st.session_state.df_void.to_csv(index=False).encode("utf-8")
        st.download_button(
...
# ===========================
    ...
    if st.session_state.img_rgb is None:
        ...
        current_inst = next(inst for inst in st.session_state.instances if inst["id"] == selected_id)
        ...
        if st.button("Sauvegarder les annotations corrigées"):
            label_path = save_corrected_annotation(
                ...
                img_shape=st.session_state.img_rgb.shape,
                save_dir=save_dir,
            )
            ...
            out_img_path = save_img_dir / st.session_state.uploaded_name
            cv2.imwrite(str(out_img_path), cv2.cvtColor(st.session_state.img_rgb, cv2.COLOR_RGB2BGR))
